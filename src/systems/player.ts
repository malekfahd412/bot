...
export class PlayerSystem {
  ...
  static adminGiveXP(
    ...
    const player =
      PlayerDB.findByDiscordId(discordId);
  ...
  static recordHeistResult(
    ...
    if (success) {
      ...
      updates.successful_heists =
        player.successful_heists + 1;
      ...
      const currentHardest =
        player.hardest_heist ?? 'easy';

      if (
        difficultyRank.indexOf(difficulty) >
        difficultyRank.indexOf(currentHardest)
      ) {

        updates.hardest_heist =
          difficulty;
      }

      this.checkHeistAchievements(
        discordId,
        player.successful_heists + 1
      );
      ...
      updates.failed_heists =
        player.failed_heists + 1;
    }
  ...
  private static checkRankAchievement(
    ...
    const rankAchievements:
      ...
      KINGPIN: [
        'rank_kingpin',
        'Kingpin',
        'Reached the pinnacle — Kingpin',
        '💎'
      ],
    };

    const entry =
      rankAchievements[rank];
  ...
  private static checkHeistAchievements(
    ...
    const milestones:
      ...
      100: [
        ...
        'Completed 100 heists',
        '💎'
      ],
    };

    const milestone =
      milestones[successfulHeists];

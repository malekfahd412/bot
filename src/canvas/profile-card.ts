...
export async function generateProfileCard(player: Player, globalRank: number): Promise<Buffer> {
  ...
  if (player.avatar_url) {
    ...
    if (avatar) {
      ...
      drawAvatarPlaceholder(ctx, displayName, avatarX, avatarY, avatarSize);
    }
  } else {
    drawAvatarPlaceholder(ctx, displayName, avatarX, avatarY, avatarSize);
  }
...
function drawAvatarPlaceholder(ctx: any, displayName: string, x: number, y: number, size: number): void {
  fillRoundedRect(ctx, x, y, size, size, size / 2, '#1A1A2E');
  ctx.font = 'bold 40px Arial';
  ctx.fillStyle = COLORS.primary;
  ctx.textAlign = 'center';
  ctx.fillText(displayName.charAt(0).toUpperCase(), x + size / 2, y + size / 2 + 14);
}

...
export async function generateLeaderboardCard(players: Player[], type: 'xp' | 'coins' = 'xp'): Promise<Buffer> {
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    fillRoundedRect(ctx, 16, ry, W - 32, ROW_H - 8, 8, rowBg);

    if (isTop3) {
      strokeRoundedRect(ctx, 16, ry, W - 32, ROW_H - 8, 8,
        i === 0 ? 'rgba(200,169,81,0.3)' : i === 1 ? 'rgba(180,180,180,0.2)' : 'rgba(180,100,50,0.2)', 1);
    }
    ...
    const displayValue = type === 'xp' ? `${formatNumber(p.xp)} XP` : formatCoins(p.coins);
    ctx.fillText(displayValue, W - 28, ry + 36);
  }

  // Footer
  const fy = HEADER_H + players.length * ROW_H + 18;
  fillRoundedRect(ctx, 0, fy, W, FOOTER_H, 0, 'rgba(200,169,81,0.06)');
  ctx.font = '11px Arial';

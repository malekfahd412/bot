...
async function generateProfileCard(player, globalRank) {
    ...
    (0, renderer_js_1.fillRoundedRect)(ctx, W - 182, 18, 164, 36, 8, 'rgba(200,169,81,0.12)');
    (0, renderer_js_1.strokeRoundedRect)(ctx, W - 182, 18, 164, 36, 8, constants_js_1.COLORS.primary, 1);
    ...
    // Username
    ctx.textAlign = 'left';
    (0, renderer_js_1.drawGlowText)(ctx, player.username, 158, 72, '#FFFFFF', constants_js_1.COLORS.primary, 28, 'bold');
    ctx.font = '14px Arial';
    ...
    const barH = 14;
    (0, renderer_js_1.drawXPBar)(ctx, barX, barY, barW, barH, xpProgress.percent);
    ctx.font = '11px Arial';
    ctx.fillStyle = constants_js_1.COLORS.textMuted;
    ctx.textAlign = 'left';
    ctx.fillText(`XP  ${(0, helpers_js_1.formatNumber)(xpProgress.current)} / ${(0, helpers_js_1.formatNumber)(xpProgress.needed)}`, barX, barY + 28);
    ctx.textAlign = 'right';
    ctx.fillStyle = constants_js_1.COLORS.primary;
    ...
    // Stats grid
    const stats = [
        { label: 'COINS', value: (0, helpers_js_1.formatCoins)(player.coins), color: constants_js_1.COLORS.gold },
        { label: 'TOTAL HEISTS', value: String(player.total_heists), color: '#FFFFFF' },
        { label: 'SUCCESS RATE', value: (0, helpers_js_1.getSuccessRate)(player.total_heists, player.successful_heists), color: constants_js_1.COLORS.success },
        { label: 'STREAK', value: `${player.streak_current} \uD83D\uDD25`, color: constants_js_1.COLORS.warning },
        { label: 'TOTAL EARNED', value: (0, helpers_js_1.formatCoins)(player.total_earnings), color: constants_js_1.COLORS.gold },
        { label: 'HARDEST JOB', value: (player.hardest_heist ?? 'NONE').toUpperCase(), color: constants_js_1.COLORS.accent },
    ];
    const cols = 3;
    ...
    stats.forEach((stat, i) => {
        ...
        (0, renderer_js_1.fillRoundedRect)(ctx, sx + 4, sy, statW - 8, rowH - 10, 8, 'rgba(255,255,255,0.03)');
        (0, renderer_js_1.strokeRoundedRect)(ctx, sx + 4, sy, statW - 8, rowH - 10, 8, 'rgba(200,169,81,0.10)', 1);
        ctx.textAlign = 'center';
    ...
    // Footer
    (0, renderer_js_1.fillRoundedRect)(ctx, 0, H - 28, W, 28, 0, 'rgba(200,169,81,0.07)');
    ctx.font = '11px Arial';
...
function drawAvatarPlaceholder(ctx, username, x, y, size) {
    (0, renderer_js_1.fillRoundedRect)(ctx, x, y, size, size, size / 2, '#1A1A2E');

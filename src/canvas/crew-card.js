...
const { getRank, getXPProgress, formatNumber, formatCoins, getSuccessRate } = require("../utils/helpers.js");
...
async function generateStatsCard(player, recentHeists) {
    ...
    drawGlowText(
        ctx,
        `📊  ${player.display_name.toUpperCase()}  —  CRIMINAL RECORD`,
        W / 2,
        45,
        '#FFFFFF',
        COLORS.primary,
    ...
    ctx.textAlign = 'left';
    ctx.fillText(
        `XP PROGRESS — ${formatNumber(xpProg.current)} / ${formatNumber(xpProg.needed)}`,
        24,
        100
    );
    ...
    ctx.fillText(
        `${Math.round(xpProg.percent * 100)}% to LVL ${player.level + 1}`,
        W - 24,
        100
    );

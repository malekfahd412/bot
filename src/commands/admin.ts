...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (sub === 'give-xp') {
    ...
    PlayerSystem.getOrCreate(target.id, target.displayName, avatarUrl);
  ...
  if (sub === 'lookup') {
    ...
    const embed = new EmbedBuilder()
      .setColor(0xC8A951)
      .setTitle(`Admin Lookup: ${player.displayName}`)
  ...
  if (sub === 'reset-streak') {
    ...
    await interaction.editReply(`✅ Streak reset for **${target.displayName}** (was **${player.streak_current} days**).`);
    return;
  }
}

import {
  ChatInputCommandInteraction, SlashCommandBuilder, AttachmentBuilder, EmbedBuilder
} from 'discord.js';
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  PlayerSystem.getOrCreate(user.id, user.username, user.displayAvatarURL({ extension: 'png', size: 256 }));
  ...
  if (sub === 'create') {
    ...
    try {
      const crew = CrewSystem.create(name, tag, user.id, description);
  ...
  if (sub === 'info') {
    ...
    if (crewName) {
      ...
      if (!found) { await interaction.editReply('❌ Crew not found.'); return; }
      crew = CrewSystem.getWithMembers(found.id);
    } else {
      crew = CrewSystem.getPlayerCrew(user.id);
    }

    if (!crew) { await interaction.editReply('❌ You are not in a crew. Create or join one first.'); return; }

    const owner = PlayerDB.findByDiscordId(crew.owner_id);
    if (!owner) { await interaction.editReply('❌ Crew data error.'); return; }

    try {
      const buffer = await generateCrewCard(crew, crew.members, owner);
      const attachment = new AttachmentBuilder(buffer, { name: 'crew.png' });
  ...
  if (sub === 'invite') {
    ...
    const player = PlayerSystem.get(user.id);
    if (!player?.crew_id) { await interaction.editReply('❌ You are not in a crew.'); return; }

    const { CrewDB } = await import('../database/db.js');
    const crew = CrewDB.findById(player.crew_id);
    if (!crew || crew.owner_id !== user.id) { await interaction.editReply('❌ Only the crew owner can invite members.'); return; }

    try {
      CrewSystem.join(player.crew_id, target.id);
